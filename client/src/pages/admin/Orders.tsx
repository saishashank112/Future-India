...
const AdminOrders = () => {
  const [orders, setOrders] = useState<Order[]>([
    ...
    {
      id: 2,
      order_code: "EXIM-2024-002",
      total_amount: 89000,
      status: "Shipped",
      payment_status: "Pending Verification",
      customer_name: "Sarah Smith",
      email: "sarah@sgcorp.sg",
      phone: "+65 9123 4567",
      created_at: new Date().toISOString(),
      address: "Marina Boulevard, Singapore",
      latitude: 1.2834,
      longitude: 103.8607,
      shipping_details: JSON.stringify({ courier: "FedEx", tracking: "456789123" })
    }
  ]);
  ...
  const updatePaymentStatus = async (id: number, status: string) => {
    try {
      await fetch(getApiUrl(`/admin/payments/${id}/status`), {
        method: 'PUT',
